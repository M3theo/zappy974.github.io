...
bool has_required_players(tile_t *tile, UI level, UI required_count)
{
    UI count = 0;
    UI res = 0;

    for (UI i = 0; tile->players[i]; i++) {
        LOG_DEBUG("Found player ID=%d level=%d", tile->players[i]->id, tile->players[i]->level);
        if (tile->players[i]->level == level)
            count++;
    }
    LOG_DEBUG("Total matching players: %u, required: %u", count, required_count);
    res = count >= required_count;
    return res;
}
...
void cmd_incantation(global_t *global, int client_id, char *args)
{
    ...
    (void)args;
    if (!players) {
        LOG_WARN("Client ID [%d]: Incantion failed, player not found.", client_id);
        dprintf(SERVER->pfds[client_id].fd, "ko\n");
        return;
    }
    if (players->level >= 8) {
        LOG_INFO("Client ID [%d]: Already at max level.", client_id);
        dprintf(SERVER->pfds[client_id].fd, "ko\n");
        return;
    }
